...
export class AnaliseService {
  ...
  async solicitarAnalise(dto: SolicitarAnaliseDto) {
    ...
    // Busca o indicador com histórico de até 12 meses [cite: 1]
    ...
    const payload = {
      ...
      dados: {
        ...
        historico: indicador.resultados.map((r) => ({
          mes: r.competencia,
          valor: r.valor,
    ...
    try {
      ...
      this.httpService.post(this.N8N_WEBHOOK_URL, payload).subscribe({
        error: (err) =>
          this.logger.error(`Erro ao notificar n8n: ${err.message}`),
      });
  ...
  async gerarAnaliseSobDemanda(dto: GerarAnaliseSobDemandaDto) {
    ...
    try {
      ...
      return {
        indicadorId,
        analiseGerada: response.data.analiseCritica,
      };
    } catch (error) {
      this.logger.error(`Timeout ou Erro no n8n: ${error.message}`);
      throw new HttpException(
