...
export function KpiCard({ indicador, onClick, isActive }: KpiCardProps) {
  ...
  const isInverseIndicator = [
    ...
    "Mortalidade",
    "Glosa",
  ].some((term) => indicador.descricao.includes(term));
      ...
      >
        <CardHeader className="pb-2">
          <div className="flex items-start justify-between">
            <CardTitle
              className="text-sm font-medium text-muted-foreground leading-tight line-clamp-2 h-10 w-full pr-2"
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                {variacao !== 0 ? (
                  <Badge
                    variant="secondary"
                    className={`text-xs font-medium px-1.5 py-0 h-5 ${variacaoPositive ? "bg-emerald-500/15 text-emerald-700 border-emerald-500/20" : "bg-red-500/15 text-red-700 border-red-500/20"}`}
                  >
                    {variacaoPositive ? (
